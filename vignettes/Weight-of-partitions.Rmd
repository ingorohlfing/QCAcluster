...
```{r, include = F}
# for internal purposes to deal with model ambiguity
library(QCA)
data("Thiem2011")
postt <- truthTable(data = Thiem2011,
                    outcome = "memberfs",
                    cond = "fedismfs, homogtyfs, powdifffs, comptvnsfs, 
                    pubsupfs, ecodpcefs",
                    n.cut = 6, incl.cut = 0.8)
pospars <- minimize(postt, include = "?")
pospars # three-fold model ambiguity
# the prime implicants are all stored without distinguishing the models
pospars$pims
# a viable way could be to extract the models and the pims of the models
# and use some string matching to identify them in $pims and use the 
# membership in the pims to calculate the weight of a partition for a
# specific model. The function then needs to have an argument for model
# selection, where the number of the model corresponds to the order
# of the models in the output
pospars$solution
```
...
postt <- truthTable(data = Thiem2011,
                    ...
                    pubsupfs, ecodpcefs",
                    n.cut = 6, incl.cut = 0.8)
posinter <- minimize(postt, include = "?",
                     dir.exp = "1, 1, 1, 1, 1, 1")
posinter #  no model ambiguity
# the pims are stored for pairs of conservative and parsimonious
# solutions
posinter$i.sol$C1P1$pims
# you need to find out how to determine intermediate model ambiguity
# here, three pairs of conservative and parsimonious solutions have
# the same intermediate solution, so no intermediate ambiguity
# if no ambiguity is given, one can work with the $C1P1$pims
# if there is ambiguity, one again needs a model selector
```
