...
The intermediate solution is derived with `partition_min_inter()`. (We decided to have a separate function for the intermediate solution for computational purposes.) For the intermediate solution, one has to specify the directional expectations. The data structure is an *unbalanced* panel with eight countries and ten years and an uneven number of observations per countries and years (by implication).
```{r}
data(Schwarz2016)
table(Schwarz2016$year, Schwarz2016$country)
```

The functions for the minimization of panel data can process balanced and unbalanced panel data without having to specify this with a function argument.
```{r}
...
The dataframe only has entries for Senders. The number of rows is larger than the number of Senders because the partition-specific solutions for the EU and the US feature model ambiguity.
